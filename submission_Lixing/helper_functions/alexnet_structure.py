...
def alex_net(X_input):
    ...
    w_dic = {  # The dictionary for all weights variables & sizes
        ...
        "pool2": [1, 2, 2, 1],
        "linear1": tf.Variable(tf.truncated_normal((400, 120), mean=mu, stddev=sigma)),
        "linear2": tf.Variable(tf.truncated_normal((120, 84), mean=mu, stddev=sigma)),
        "linear3": tf.Variable(tf.truncated_normal((84, 43), mean=mu, stddev=sigma))
    }

    b_dic = {  # The dictionary for all biases
        "conv1": tf.Variable(tf.zeros((6))),
        "conv2": tf.Variable(tf.zeros((16))),
        "linear1": tf.Variable(tf.zeros((120))),
        "linear2": tf.Variable(tf.zeros((84))),
        "linear3": tf.Variable(tf.zeros((43)))
    }
